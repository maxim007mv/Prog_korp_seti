...
export const bookingsApi = {
  ...
  searchBookings: (params: BookingSearchParams) =>
    apiClient.get<Booking[]>('/bookings/search', params),
...
export const reportsApi = {
  // Дашборд KPI
  getDashboardKpi: () => apiClient.get<DashboardKpi>('/reports/dashboard'),

  // Отчёт по выручке (только закрытые заказы)
  getRevenueReport: (params: ReportParams) =>
    apiClient.get<RevenueReport>('/reports/revenue', params),

  // Отчёт по официантам
  getWaitersReport: (params: ReportParams) =>
    apiClient.get<WaiterReport>('/reports/waiters', params),

  // Отчёт популярности блюд
  getPopularDishesReport: (params: ReportParams) =>
    apiClient.get<PopularDishesReport>('/reports/popular', params),
};
